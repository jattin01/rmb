                        ...
                        <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                            <div class="row mt-sm-4 mt-3">
                                ...
                                <div class="col-md-8 text-sm-right">
                                    ...
                                    <a href="{{route('settings.drivers.create')}}" class="btn btn-success mr-2">Create New</a>
                                    {{-- <button type="button" class="btn export-btn mr-2">Export</button> --}}
                                    <a type="button" class="btn export-btn mr-2"
                                    href="{{ route('settings.drivers.export', ['search' => request('search'), 'group_company_id' => request('group_company_id')]) }}"
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                ...
                                                <div class="select-box form-group">
                                                    ...
                                                    <select class="form-control select-contentbox"
                                                        ...
                                                        @foreach ($drivers as $driver)
                                                            <option
                                                                value="{{ $driver->group_company?->id }}">
                                                                {{ $driver->group_company?->comp_name }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="row align-items-center mt-3">
                                                    <div class="col-md-6 col-4">
                                                        <a class="reset-text"
                                                            href="{{ url('settings/drivers') }}">Reset
                                                        </a>
                                                    </div>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table general-table">
                                            ...
                                            <tbody>
                                                @foreach(@$drivers as $driver)
                                                <tr>
                                                    <td>{{$driver->group_company ?-> comp_name}}</td>
                                                    <td>{{$driver->employee_code}}</td>
                                                    <td>{{$driver->name}}</td>
                                                    <td>{{$driver->phone}}</td>
                                                    <td>{{$driver->email_id}}</td>
                                                    <td>{{$driver->username}}</td>
                                                    <td>{{$driver->user_role}}</td>
                                                    <td class="{{$driver->status == 'Active' ? 'table-activetext' : 'table-inactivetext'}}">{{$driver->status}}</td>
                                                    <td class="gray-text">
                                                    <div class="d-flex align-items-center justify-content-between" style = "margin:0.4rem;">
																<div class="dropdown more-drop">
																	...
																	<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
																	  <a class="dropdown-item" href="{{route('settings.drivers.edit', ['driver_id' => $driver->id])}}">Details</a>
																	</div>
																  </div>
																</div>
                                                    </td>
