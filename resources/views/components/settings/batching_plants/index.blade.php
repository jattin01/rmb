                ...
                <div class="col-md-12">
                    <div class="tab-content" id="myTabContent">
                        <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                            <div class="row mt-sm-5 mt-3">
                                ...
                                <div class="col-md-8 text-sm-right">
                                    ...
                                    <a href="{{route('settings.batchingPlants.create')}}" class="btn btn-success mr-2">Create New</a>
                                    {{-- <button type="button" class="btn export-btn mr-2">Export</button> --}}
                                    <a type="button" class="btn export-btn mr-2"
                                    href="{{ route('settings.batchingPlants.export', ['search' => request('search'), 'group_company_id' => request('group_company_id')]) }}"
                                    ...
                                    <div class="dropdown drop-mainbox">
                                        ...
                                        <form method="GET">
                                            <input type="hidden" id = "project_id_filter" name="project_id" value="{{request()->project_id}}">
                                            ...
                                            <div class="select-box form-group">
                                                    ...
                                                    <select class="form-control select-contentbox"
                                                        ...
                                                        <option value="">Select </option>
                                                        @foreach ($batchingPlants as $batchingPlant)
                                                            <option value="{{$batchingPlant -> company_location ?-> id}}">
                                                                {{$batchingPlant -> company_location ?-> location}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="row align-items-center mt-3">
                                                    <div class="col-md-6 col-4">
                                                        <a class="reset-text"
                                                        href="{{ url('settings/batching-plants') }}">Reset
                                                        </a>
                                                    </div>
                        ...
                        <table class="table general-table">
                            ...
                            <tbody>
                                @forelse ($batchingPlants as $batchingPlant)
                                <tr>
                                    <td>{{$batchingPlant -> group_company ?-> comp_name}}</td>
                                    <td>{{$batchingPlant -> plant_name}}</td>
                                    <td>{{$batchingPlant -> long_name}}</td>
                                    <td>{{$batchingPlant -> company_location ?-> location}}</td>
                                    <td>{{$batchingPlant -> capacity}} CUM</td>
                                    <td>{{$batchingPlant -> description}}</td>
                                    <td class="{{$batchingPlant->status == 'Active' ? 'table-activetext' : 'table-inactivetext'}}">{{$batchingPlant -> status}}</td>
                                    <td>
                                    <div class="d-flex align-items-center justify-content-between" style = "margin:0.4rem;">
																<div class="dropdown more-drop">
																	...
																	<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
																	  <a class="dropdown-item" href="{{route('settings.batchingPlants.edit', ['plantId' => $batchingPlant->id])}}">
                                                                      Details
                                                                    </a>
																	</div>
																  </div>
                    ...
                    {{ $batchingPlants->links('partials.pagination') }}
