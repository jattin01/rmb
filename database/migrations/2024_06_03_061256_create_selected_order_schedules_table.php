...
return new class extends Migration
{
    ...
    {
        Schema::create('selected_order_schedules', function (Blueprint $table) {
            $table->id();
            $table->foreignId('order_id')->constrained("selected_orders")->index("order_s_sch_order_index");
            $table->foreignId('group_company_id')->constrained("group_companies")->index("order_s_sch_grp_index");
            $table->foreignId("user_id")->constrained()->index("order_s_sch_user_index");
            $table->date("schedule_date");
            $table->string("order_no");
            $table->string("location");
            $table->integer("trip");
            $table->string("mix_code");
            $table->foreignId('cust_product_id')->nullable()->constrained("customer_products")->index("order_s_sch_prod_index");
            $table->string("batching_plant");
            $table->foreignId('batching_plant_id')->nullable()->constrained("batching_plants")->index("order_s_sch_bp_index");
            $table->string("transit_mixer");
            $table->foreignId('transit_mixer_id')->nullable()->constrained("transit_mixers")->index("order_s_sch_tm_index");
            $table->string("pump") -> nullable();
            $table->foreignId('pump_id')->nullable()->constrained("pumps")->index("order_s_sch_pump_index");
