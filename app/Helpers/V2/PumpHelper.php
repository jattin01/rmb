...
// dd(collect($assinedPump)->flatten()->toArray());
        ...
        foreach($pumps as $pumpKey => $pump) {
            ...
            if (Carbon::parse($pump['free_from']) -> gt(Carbon::parse($pump_start_time))) continue;
            
            if(Carbon::parse($pump['free_from']) -> gt($pump_end_time) ) continue;

            if(Carbon::parse($pump['free_upto']) -> lt(Carbon::parse($pump_start_time))) continue;
            
            if(Carbon::parse($pump['free_upto']) -> lt($pump_end_time)) continue;
