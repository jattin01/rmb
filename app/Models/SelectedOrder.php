...
class SelectedOrder extends Model
{
    ...
    public function schedule()
    {
        return $this->hasMany(SelectedOrderSchedule::class, 'order_id', 'id')
            ->leftJoin("transit_mixers", function ($query) {
                ...
                "selected_order_schedules.deviation",
    ...
    {
        $toleranceMinutes = (int) ceil(($this->interval * $this->tolerance) / 100);
        $maxInterval      = $this->min_interval;
        if($this->flexibility)
            return 40;
        else
        return $maxInterval;
