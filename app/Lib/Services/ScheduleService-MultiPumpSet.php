...
class ScheduleService
{
    ...
    private function processOrder($order, ScheduleData &$scheduleData, ScheduleData &$generatedScheduleData, $orderKey)
    {
        ...
        foreach ($locations as $location) {
            ...
            if ($scheduleData->is_completed) {
                if ($order->pump) {
                    $this->assignPump($order, $scheduleData, $location);
                }
                break;
            }
        }
    ...
    private function processTrips($order, ScheduleData &$scheduleData, ScheduleData &$generatedScheduleData, $location, $orderKey)
    {
        ...
        $toleranceMinutes = $scheduleData->order_interval;
        ...
        while ($quantity > 0) {
            ...
            // -------- SHIFT VALIDATION ----------
            if (
                $scheduleData->loading_start->gt($scheduleData->shift_end) ||
                $scheduleData->loading_start->lt($scheduleData->shift_start)
            ) {
            ...
            if ($this->allResourcesAssigned($scheduleData)) {
                ...
                // -------- SUCCESS CASE ----------
                $scheduleData->batching_qty = min(
                    $scheduleData->transit_mixer['data']['truck_capacity'],
                    $quantity
                );
                ...
                $found = $this->tryNearbySlotWithoutReset(
                    $order,
                    $scheduleData,
                    $location,
                    $trip,
                ...
                $nextDeliveryTime = ($scheduleData->phase === 2)
                    ? $scheduleData->order_start->copy()->subMinutes(1)
                    : $scheduleData->order_start->copy()->addMinutes(1);
    ...
    private function tryNearbySlotWithoutReset($order, ScheduleData &$scheduleData, $location, int $trip, int $toleranceMinutes = 10): bool
    {
        $originalDelivery = $scheduleData->delivery_time->copy();

        for ($i = 1; $i <= $toleranceMinutes; $i++) {

            foreach ([$i, -$i] as $offset) {

                $candidateTime = $originalDelivery->copy()->addMinutes($offset);

                if (
                    $candidateTime->lt($scheduleData->shift_start) ||
                    $candidateTime->gt($scheduleData->shift_end)
                ) {
                    continue;
                }

                $scheduleData->delivery_time = $candidateTime;
        ...
        // revert
        $scheduleData->delivery_time = $originalDelivery;
        $this->updateSchedule($scheduleData, $order);

        return false;
    }
    private function assignPump($order, ScheduleData &$scheduleData, $location): void
    {
        // No pump requirement
        ...
        for ($p = 0; $p < $pumpsRequired; $p++) {
            ...
            $groupTrips = [];
            for ($i = $p; $i < $totalTrips; $i += $pumpsRequired) {
                $groupTrips[] = $trips[$i];
            }
            if (empty($groupTrips))
                continue;

            // Sort group by pouring_start to avoid start/end swapping
            usort($groupTrips, function ($a, $b) {
                return Carbon::parse($a['pouring_start'])->timestamp <=> Carbon::parse($b['pouring_start'])->timestamp;
            });

            $first = $groupTrips[0];
            $last = $groupTrips[count($groupTrips) - 1];

            // Pouring window differs per pump group
            $groupPourStart = Carbon::parse($first['pouring_start']);
            $groupPourEnd = Carbon::parse($last['pouring_end']);

            // Midnight rollover guard
            if ($groupPourEnd->lt($groupPourStart)) {
                $groupPourEnd->addDay();
            }

            // Pump end time should be LAST return_end of that group
            $groupPumpEndTime = Carbon::parse($last['return_end']);

            // Midnight rollover guard
            if ($pumpsRequired > 1)
                $groupPumpLoadingTime = Carbon::parse($trips[$totalTrips - 1]['loading_start']);
            else
                $groupPumpLoadingTime = Carbon::parse($trips[0]['loading_start']);
            if ($groupPumpEndTime->lt($groupPumpLoadingTime)) {
                $groupPumpEndTime->addDay();
            }

            // Pump sequence (1..pumpsRequired)
            $pumpSeq = $p + 1;
            ...
            Log::debug('Slots for order '.$order->order_no, $scheduleData->pump_busy_slots);
            ...
            $scheduleData->selected_order_pump_schedules[$pumpName] = [
                ...
                'trip' => count($groupTrips),
                'batching_qty' => array_sum(array_map(fn($x) => (float) ($x['batching_qty'] ?? 0), $groupTrips)),
