...
class ScheduleService
{
    ...
    public function generateSchedule(ScheduleData &$scheduleData)
    {
        try {
            ...
            $scheduleData->original_trips = $allTrips;


            Log::info("[CHRONO_SCHED] ── Step 2: Total trips generated: " . count($allTrips) . " ──");
            $scheduleData->original_tms = $scheduleData->tms_availability;
            $scheduleData->original_bps = $scheduleData->bps_availability;
    ...
    private function scheduleTripsChronologically(ScheduleData &$scheduleData, array $sortedTrips, $allOrders): void
    {
        ...
        $orderCompleted        = [];  // order_no → true once all qty delivered
        $orderTripCounter      = [];  // order_no → sequential trip number actually scheduled

        // ── Partial-order retry tracking ───────────────────────────────────────
        // When a mid-order trip (>1) fails, we bump the order's delay by +1 min,
        // roll back its committed trips, and re-inject shifted trips into the
        // queue. No recursion, no pool reset for other orders.
        $orderDelayAttempt = []; // order_no → current retry delay in minutes
        $orderMaxDelay     = []; // order_no → max_delay_minutes ceiling (cached)

        // ── Working queue (not foreach — we need to append during iteration) ──
        $queue = $sortedTrips;
        $qi    = 0;
        ...
        while ($qi < count($queue)) {
            ...
            if (!isset($orderRemainingQty[$orderNo])) {
                ...
                $orderDelayAttempt[$orderNo] = 0;
                $orderMaxDelay[$orderNo]     = (int) ($tripData['max_delay_minutes'] ?? 0);
            }
            ...
            $tripScheduled = false;
            if ($tripData['trip'] === 1) {
                $delay = Carbon::parse($order->delivery_date)->diffInMinutes(Carbon::parse($tripData['pouring_start']), false);
                $retryOffset = $delay;
            }
            ...
            if (!$tripScheduled && !isset($orderFailed[$orderNo])) {
                ...
                $currentDelay = $orderDelayAttempt[$orderNo] ?? 0;
                $maxDelay     = $orderMaxDelay[$orderNo] ?? 0;

                // ── PARTIAL ORDER DETECTED (trip >1 failed) ─────────────────────
                // Rollback whatever this order committed, bump delay +1 min,
                // re-inject all its trips (shifted) back into the queue.
                if ($tripData['trip'] > 1 && $currentDelay < $maxDelay) {
                    $newDelay = $currentDelay + 1;
                    $orderDelayAttempt[$orderNo] = $newDelay;

                    $committedCount = count($orderSchedules[$orderNo] ?? []);
                    Log::warning("[PARTIAL] order={$orderNo} failed at trip={$tripData['trip']} — "
                        . "rolling back {$committedCount} committed trip(s), "
                        . "retrying with delay={$newDelay}min (max={$maxDelay}min)");

                    // 1) Rollback: clear busy slots + journal + state for THIS order only
                    $this->rollbackOrder(
                        $scheduleData, $orderNo,
                        $orderSchedules, $orderDelivered, $orderRemainingQty,
                        $orderCompleted, $pumpAssigned, $bestPlantPerOrderTrip,
                        $orderCumulativeDelay, $order
                    );

                    // 2) Re-inject: pull this order's original trips (pre-drift),
                    //    shift each by the full new delay, append them back into
                    //    the queue in chronological order.
                    $reinjected = [];
                    foreach ($sortedTrips as $t) {
                        if ($t['order_no'] !== $orderNo) continue;
                        $reinjected[] = $this->shiftTripByMinutes($t, $newDelay, $order, $scheduleData);
                    }
                    usort($reinjected, fn($a, $b) => $a['trip'] <=> $b['trip']);

                    // Find chronological insertion point in the unprocessed portion of $queue
                    $insertAt           = $qi;
                    $trip1PouringStart  = Carbon::parse($reinjected[0]['pouring_start']);
                    for ($j = $qi; $j < count($queue); $j++) {
                        if (Carbon::parse($queue[$j]['pouring_start'])->gte($trip1PouringStart)) {
                            $insertAt = $j;
                            break;
                        }
                        $insertAt = $j + 1;
                    }
                    array_splice($queue, $insertAt, 0, $reinjected);

                    Log::info("[PARTIAL_RETRY] order={$orderNo} re-injected "
                        . count($reinjected) . " trips at queue pos={$insertAt} "
                        . "with delay={$newDelay}min");

                    continue; // next iteration of the while loop picks up re-injected trip 1
                }

                // ── HARD FAIL: trip 1 failed OR delay budget exhausted ─────────
                $orderFailed[$orderNo] = "order={$orderNo} trip={$tripData['trip']} "
                    . "could not be scheduled after {$delayMinutes}min of retries "
                    . "(delay_attempt={$currentDelay}min, min_interval={$minInterval}, "
                    . "max_interval={$maxInterval}, tolerance={$tolerancePercent}%).";
                Log::error("[CHRONO_SCHED] ✗ " . $orderFailed[$orderNo]);

                // Any commits that leaked through before hard-fail must be cleaned up too
                $this->rollbackOrder(
                    $scheduleData, $orderNo,
                    $orderSchedules, $orderDelivered, $orderRemainingQty,
                    $orderCompleted, $pumpAssigned, $bestPlantPerOrderTrip,
    ...
    /**
     * Roll back every commitment made so far for $orderNo.
     * Called when a mid-order trip fails (partial) so we can retry from trip 1
     * with a bigger delay, or when the order hard-fails entirely.
     *
     * Affects ONLY the given order — other orders' state is untouched.
     *
     * Clears:
     *  - busy-slot rows filtered by order_no
     *  - tms/bps availability splits this order caused (via order_journal)
     *  - in-memory schedule entries, delivered qty, pump reservation
     *  - per-order scheduling state so it starts fresh on re-inject
     */
    private function rollbackOrder(
        ScheduleData &$scheduleData,
        string $orderNo,
        array &$orderSchedules,
        array &$orderDelivered,
        array &$orderRemainingQty,
        array &$orderCompleted,
        array &$pumpAssigned,
        array &$bestPlantPerOrderTrip,
        array &$orderCumulativeDelay,
        $order
    ): void {
        $tripsCleared = count($orderSchedules[$orderNo] ?? []);

        // 1. Remove busy-slot rows tagged with this order
        $scheduleData->plant_busy_slots = array_values(array_filter(
            $scheduleData->plant_busy_slots,
            fn($s) => ($s['order_no'] ?? null) !== $orderNo
        ));
        $scheduleData->truck_busy_slots = array_values(array_filter(
            $scheduleData->truck_busy_slots,
            fn($s) => ($s['order_no'] ?? null) !== $orderNo
        ));
        $scheduleData->pump_busy_slots_unset = array_values(array_filter(
            $scheduleData->pump_busy_slots_unset,
            fn($s) => ($s['order_no'] ?? null) !== $orderNo
        ));

        // 2. Rebuild tms_availability + bps_availability from surviving busy slots.
        //    We can't just "restore original rows from journal" because other orders
        //    may have chained mutations onto the same keys after this order. Instead:
        //     a) Reset pools to their pristine pre-scheduling snapshot.
        //     b) Re-apply every busy-slot that still belongs to a non-rolled-back order.
        //
        //    This produces a consistent state regardless of interleaving.
        $scheduleData->tms_availability = $scheduleData->original_tms;
        $scheduleData->bps_availability = $scheduleData->original_bps;

        // Re-apply every surviving plant_busy_slot
        foreach ($scheduleData->plant_busy_slots as $pbs) {
            $this->applyPlantSlotToAvailability($scheduleData, $pbs);
        }
        // Re-apply every surviving truck_busy_slot
        foreach ($scheduleData->truck_busy_slots as $tbs) {
            $this->applyTruckSlotToAvailability($scheduleData, $tbs);
        }

        unset($scheduleData->order_journal[$orderNo]);

        // 3. Clear per-order scheduling state
        unset(
            $orderSchedules[$orderNo],
            $orderDelivered[$orderNo],
            $pumpAssigned[$orderNo],
            $bestPlantPerOrderTrip[$orderNo],
            $orderCumulativeDelay[$orderNo],
            $orderCompleted[$orderNo]
        );

        // 4. Reset remaining qty to full order quantity for the next attempt
        $orderRemainingQty[$orderNo] = (int) $order->quantity;

        Log::info("[ROLLBACK] order={$orderNo} cleared {$tripsCleared} committed trip(s) "
            . "and restored pool state.");
    }

    /**
     * Carve a plant busy slot out of bps_availability.
     * Finds the free window containing the slot, clips its free_upto to slot start,
     * and appends a new "after slot" window from slot end. Mirrors the logic inside
     * updateResourcePoolsOnly() so rebuilds are deterministic.
     */
    private function applyPlantSlotToAvailability(ScheduleData &$scheduleData, array $slot): void
    {
        $plantId  = $slot['plant_id'] ?? null;
        $slotStart = Carbon::parse($slot['start']);
        $slotEnd   = Carbon::parse($slot['end']);
        if (!$plantId) return;

        foreach ($scheduleData->bps_availability as $k => $row) {
            if (($row['plant_name'] ?? null) !== $plantId) continue;
            $freeFrom = Carbon::parse($row['free_from']);
            $freeUpto = Carbon::parse($row['free_upto']);
            if ($slotStart->lt($freeFrom) || $slotStart->gte($freeUpto)) continue;

            // Clip this window to end just before the slot
            $scheduleData->bps_availability[$k]['free_upto'] = $slotStart->copy()->addSeconds();
            if ($scheduleData->bps_availability[$k]['free_upto'] <= $freeFrom) {
                unset($scheduleData->bps_availability[$k]);
            }

            // Append the tail after the slot ends
            $scheduleData->bps_availability[] = [
                'plant_name'     => $row['plant_name'],
                'plant_capacity' => $row['plant_capacity'] ?? null,
                'free_from'      => $slotEnd->copy()->subSeconds(),
                'free_upto'      => $freeUpto,
                'location'       => $row['location'] ?? ($slot['location'] ?? null),
            ];
            return;
        }
    }
    ...
    /**
     * Carve a truck busy slot out of tms_availability (same logic as above but for trucks).
     */
    private function applyTruckSlotToAvailability(ScheduleData &$scheduleData, array $slot): void
    {
        ...
        foreach ($scheduleData->tms_availability as $k => $row) {
            if (($row['truck_name'] ?? null) !== $truckId) continue;
            $freeFrom = Carbon::parse($row['free_from']);
            $freeUpto = Carbon::parse($row['free_upto']);
            if ($slotStart->lt($freeFrom) || $slotStart->gte($freeUpto)) continue;

            $scheduleData->tms_availability[$k]['free_upto'] =
                $slotStart->copy()->addSeconds()->format('Y-m-d H:i:s');
            if ($scheduleData->tms_availability[$k]['free_upto']
                <= $scheduleData->tms_availability[$k]['free_from']) {
                unset($scheduleData->tms_availability[$k]);
            }

            $scheduleData->tms_availability[] = [
                'truck_name'     => $row['truck_name'],
                'truck_capacity' => $row['truck_capacity'] ?? ($slot['cap'] ?? null),
                'loading_time'   => $row['loading_time'] ?? null,
                'free_from'      => $slotEnd->copy()->subSeconds()->format('Y-m-d H:i:s'),
                'free_upto'      => $freeUpto,
    ...
    private function updateResourcePoolsOnly(ScheduleData &$scheduleData, $order, string $location): void
    {
        ...
        $journalEntry['truck_added_key'] = array_key_last($scheduleData->tms_availability);
        ...
        $journalEntry['plant_added_key'] = array_key_last($scheduleData->bps_availability);

        // Register journal entry under this order for rollback
        $orderNo = $scheduleData->order_no;
        if (!isset($scheduleData->order_journal)) {
            $scheduleData->order_journal = [];
        }
        if (!isset($scheduleData->order_journal[$orderNo])) {
            $scheduleData->order_journal[$orderNo] = [];
        }
        $scheduleData->order_journal[$orderNo][] = $journalEntry;
    ...
    private function delayOrder(array $trips, string $orderNo, int $minutes = 1): array
    {
        $timeFields = [
            'loading_start',
            'loading_end',
        ...
        foreach ($trips as &$trip) {
            if ($trip['order_no'] === $orderNo) {

                foreach ($timeFields as $field) {
                    if (!empty($trip[$field])) {
                        $trip[$field] = Carbon::parse($trip[$field])
                            ->addMinutes($minutes);
                    }
                }
            }
