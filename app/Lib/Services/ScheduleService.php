...
class ScheduleService
{
    ...
    private function resetOrderVariables(ScheduleData &$scheduleData, $order, $truckQty = 8)
    {
        ...
        $pouring_start = $scheduleData->insp_end->copy()->addMinute();
        // if ($scheduleData->current_interval < 0)
        //     $pouring_start = $scheduleData->insp_end->copy()->addMinutes(abs($scheduleData->current_interval));
        $scheduleData->pouring_start = $pouring_start;
    ...
    private function processTrips($order, ScheduleData &$scheduleData, ScheduleData &$generatedScheduleData, $location, $orderKey)
    {
        ...
        while ($quantity > 0) {
            ...
            if ($this->allResourcesAssigned($scheduleData)) {
                ...
                if (abs($scheduleData->current_interval) <= $scheduleData->order_interval) {
                    if ($scheduleData->current_interval > 0) {
                        $scheduleData->current_interval = -$scheduleData->current_interval;
                    } else {
                        $scheduleData->current_interval = abs($scheduleData->current_interval) + 1;
                    }
                } else {
                    if ($scheduleData->phase === 2) {
                ...
                if ($scheduleData->trip > 1) {
                    ...
                    if ($scheduleData->current_interval <= 0)
                        $nextDeliveryTime = $scheduleData->order_start->copy()->subMinutes(abs($scheduleData->current_interval));
                    else
                        $nextDeliveryTime = $scheduleData->order_start->copy()->addMinutes(abs($scheduleData->current_interval));
                    $shiftEndExit = $scheduleData->shift_end_exit;
                    $phase = $scheduleData->phase;
                    $earlyTrip = null;
                    $lateTrip = null;
