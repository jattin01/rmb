...
class ScheduleService
{
    ...
    private function scheduleTripsChronologically(ScheduleData &$scheduleData, array $sortedTrips, $allOrders): void
    {
        ...
        $pumpAssigned = []; // pump assignment status per order to ensure consistency across trips
        ...
        foreach ($sortedTrips as $tripData) {
            ...
            while ($retryOffset <= $maxRetryMinutes) {
                ...
                if ($order->pump && $scheduleData->trip === 1) {
                    ...
                    // Guard: don't add duplicate slot if already reserved for this order
                    ...
                    if (!$alreadyReserved) {
                        ...
                        if ($delay_time > 0) {
                            ...
                            if ($retryOffset + $delay_time > $maxRetryMinutes) {
                                $orderFailed[$orderNo] = "Pump unavailable. Suggested order time: " .
                                    $delay['pouring_start']->format('h:i A') .
                                    " | Pump will be assigned if ordered by this time. " .
                                    " | Max allowed delay: " .
                ...
                if ($order->pump && $scheduleData->trip > 1 && !$pumpAssigned[$orderNo]) {
                    $retryOffset++;
                    continue;
                }

                // ── STEP 2: PLANT — only reached if pump is confirmed free ───────────
                ...
                // ── STEP 3: Update pump slot end on final trip ────────────────────────
                if ($scheduleData->trip === $totalTrips && $order->pump) {
