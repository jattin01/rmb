...
class ScheduleService
{
    ...
    private function processOrder($order, ScheduleData &$scheduleData, ScheduleData &$generatedScheduleData, $orderKey)
    {
        ...
        if($scheduleData->shift_end_exit === 1) {
            ...
            Log::info("Shift End Exit: 5 NEXT - ".$nextDeliveryTime .'-'.$scheduleData->shift_start );

            if($nextDeliveryTime->lte($scheduleData->shift_start)) {
               Log::info("Day Start time limit" );
        ...
        if($scheduleData->shift_end_exit === 2) {
            ...
            Log::info("Shift End Exit: 5 - 2 - ".$nextDeliveryTime .'-'.$scheduleData->shift_start );

            if($nextDeliveryTime->gte($scheduleData->shift_end)) {
               Log::info("Day End time limit" );
    ...
    private function adjustScheduleBasedOnAvailability($order, ScheduleData $scheduleData, $location, $orderKey)
    {

        // $adjustmentTime = $scheduleData->sch_adj_from ?? 0;
        // $deliveryDateN = $scheduleData->delivery_time->addMinutes($adjustmentTime);
        // $deliveryDateP = $scheduleData->delivery_time->subMinutes($adjustmentTime);


        // $deliveryDate = $deliveryDateN;

        // $scheduleData->delivery_time = $deliveryDate;
        ...
        // if ($scheduleData->shift_end_exit === 1) {
        //     Log::info("Shift End Exit: " . $scheduleData->shift_end_exit);
        //     break;
        // }

        // if ($scheduleData->shift_end_exit === 2) {
        //     Log::info("Shift Start Exit: " . $scheduleData->shift_end_exit);
        //     break;
        // }


        // if ($scheduleData->is_completed) {
        //     Log::info("Order Completed: " . $scheduleData->is_completed);
        //     break;
        // }
    ...
    private function processTrips($order, ScheduleData &$scheduleData, $location, $orderKey)
    {
        ...
        while ($quantity > 0) {
            ...
            if($scheduleData->loading_start->gt($scheduleData->shift_end)) {
                $scheduleData->shift_end_exit = 1;
                Log::info("Shift End Exit: 2 " . $scheduleData->shift_end_exit);
                break;
            }
            // if($scheduleData->loading_start->lte($scheduleData->shift_start)) {
            //     $scheduleData->shift_end_exit = 1;
            //     Log::info("Shift Start Exit: 2 " . $scheduleData->shift_end_exit);
            //     break;
            // }


            // if($scheduleData->delivery_time->gt($scheduleData->shift_end)) {
            //     $scheduleData->shift_end_exit = 1;
            //     Log::info("Shift End Exit: 2-1 " . $scheduleData->delivery_time);
            //     break;
            // }

            Log::info("Processing Trip: " . $trip . " for Order: " . $order->order_no . " quantity: " . $quantity);

             // If pump_qty > 1, allow parallel trips
            // $parallelTrips = $order->pump_qty > 1 ? $order->pump_qty : 1;

            // if($trip == 37) {
            //     dd( "Trip 32", $scheduleData);
            // }

            // for ($i = 0; $i < $parallelTrips; $i++) {
            ...
            // dd('pt, sc', $scheduleData);

            // if($order->order_no == '11300' && $scheduleData->loading_start->gt(Carbon::parse("2025-03-01 22:00:00"))) {
            //     dd($scheduleData);
            // }
            if ($this->allResourcesAssigned($scheduleData)) {
                Log::info("All Resources Assigned for Trip: " . $trip);
                ...
                Log::info("Resource Not Found: " . $trip);
                ...
                if($scheduleData->trip > 1) {
                    Log::info('trip update again-'.$scheduleData->trip);
                ...
                if($scheduleData->loading_start->gt($scheduleData->shift_end)) {
                    $scheduleData->shift_end_exit = 1;
                    Log::info("Shift End Exit: 3 " . $scheduleData->shift_end_exit);
                    break;
                }
                // if($scheduleData->order_no == '11300') {
                //     dd($scheduleData);
                // }
            }
