...
class ScheduleService
{
        ...
        foreach ($allOrders as $order) {
            ...
            for ($trip = 1; $trip <= $totalTrips; $trip++) {
                ...
                $allTrips[] = [
                    ...
                    'max_delay_interval' => $order->max_delay_interval,
                    'insp_end'       => $inspEnd,
                    ...
                    'base_interval'=>$order->base_interval,
                ];
                $remainingQty -= $batchQty;
                $orderIndex++;
                Log::info("[GEN_TRIPS] order={$order->order_no} trip={$trip}/{$totalTrips} "
                    . "qty={$batchQty} loading_start=" . $loadingStart->format('H:i') . " loading_end=" . $loadingEnd->format('H:i'));
            }
        }
    ...
    private function scheduleTripsChronologically(ScheduleData &$scheduleData, array $sortedTrips, $allOrders): void
    {
        ...
        foreach ($sortedTrips as $tripData) {
            ...
            $maxRetryMinutes = $tripData['trip'] > 1 ? $tripData['max_delay_interval']
            ...
            // ── Build retry offset sequence ──
            // Trip 1: forward only (0, 1, 2, 3, … maxRetryMinutes)
            // Trip > 1: bidirectional bounded by min_interval / max_interval
            //   e.g. base interval=9, min=7, max=12 → can go -2 (earlier) to +3 (later)
            //   pattern: 0, -1, +1, -2, +2, +3
            $retryOffsets = [0];
            if ($tripData['trip'] > 1) {
                $baseInterval = (int) ($tripData['base_interval'] ?? 0);
                $minInterval  = (int) ($tripData['min_interval'] ?? $baseInterval);
                $maxInterval  = (int) ($tripData['max_interval'] ?? $baseInterval);
                $maxNegative  = $baseInterval - $minInterval;  // how many minutes we can go earlier
                $maxPositive  = $maxInterval - $baseInterval;  // how many minutes we can go later
                $maxBound     = max($maxNegative, $maxPositive);
                for ($step = 1; $step <= $maxBound; $step++) {
                    if ($step <= $maxNegative) {
                        $retryOffsets[] = -$step;
                    }
                    if ($step <= $maxPositive) {
                ...
                for ($step = 1; $step <= $maxRetryMinutes; $step++) {
                    $retryOffsets[] = $step;   // forward only
                }
            }
            $tripScheduled  = false;
            $retryIdx       = 0;
            $triedOffsets   = [];          // track already-visited offsets
            while ($retryIdx < count($retryOffsets)) {
                $retryOffset = $retryOffsets[$retryIdx];
                $retryIdx++;
                // Skip if this offset exceeds the allowed window in either direction
                if (abs($retryOffset) > $maxRetryMinutes) {
                    continue;
                }
                // Skip if we already tried this exact offset (can happen after jump-ahead)
                if (isset($triedOffsets[$retryOffset])) {
                    continue;
                }
                $triedOffsets[$retryOffset] = true;
                ...
                $this->applyTripToScheduleData(
                    $scheduleData,
                    $currentTrip,
                    $orderLastPouringEnd[$orderNo] ?? null
                );
                if ($order->pump && $scheduleData->trip === 1) {
                    ...
                    if (!$alreadyReserved) {
                        ...
                        if ($delay_time > 0) {
                            ...
                            // Check if ANY remaining offset can still fit
                            $canStillFit = false;
                            for ($fi = $retryIdx; $fi < count($retryOffsets); $fi++) {
                                if (abs($retryOffsets[$fi]) <= $maxRetryMinutes) {
                                    $canStillFit = true;
                                    break;
                                }
                            }
                            if (!$canStillFit) {
                                $orderFailed[$orderNo] = "Pump unavailable. Within Max allowed delay time: " .
                            ...
                            // Continue to next offset in the alternating sequence
                            continue;
                        }
                ...
                if ($order->pump && $scheduleData->trip > 1 && empty($pumpAssigned[$orderNo])) {
                    continue; // try next alternating offset
                }
                ...
                if (!isset($scheduleData->transit_mixer['data']['truck_name'])) {
                    ...
                    // Continue to next alternating offset
                    continue;
                }
                ...
                Log::info("{$scheduleData->batching_plant['data']['plant_name']} plant assigned order: {$order->order_no} trip {$currentTrip['trip']} loading_start = {$scheduleData->loading_start} loading_ends = {$scheduleData->loading_end}");
                ...
                if (!empty($currentTrip['order_pump'])) {
                    $orderLastPouringEnd[$orderNo] = $scheduleData->pouring_end instanceof Carbon
                        ? $scheduleData->pouring_end->copy()
                        : Carbon::parse($scheduleData->pouring_end);
                }
                $tripScheduled = true;
                break;
            }
    ...
    private function applyTripToScheduleData(
        ScheduleData &$scheduleData,
        array $trip,
        ?Carbon $previousPouringEnd = null
    ): void {
        // ── Copy all pre-computed timings verbatim ───────────────────────────
        ...
        // ── Reset waiting fields; re-populate only when overlap is detected ──
        $scheduleData->waiting_start = null;
        $scheduleData->waiting_end   = null;
        $scheduleData->waiting_time  = 0;

        // ── Waiting-time injection ───────────────────────────────────────────
        // $previousPouringEnd is keyed by order_no in the caller, so a non-null
        // value already guarantees there is a confirmed earlier trip for THIS
        // specific order.  No trip-number check is needed — if this is the first
        // trip, the caller simply passes null and the block is skipped entirely.
        if (!empty($trip['order_pump']) && $previousPouringEnd !== null) {
            $currentPouringStart = $trip['pouring_start'] instanceof Carbon
                ? $trip['pouring_start']->copy()
                : Carbon::parse($trip['pouring_start']);

            $prevEnd = $previousPouringEnd instanceof Carbon
                ? $previousPouringEnd->copy()
                : Carbon::parse($previousPouringEnd);

            // Overlap exists when the previous trip's pour hasn't finished yet
            if ($currentPouringStart->lt($prevEnd)) {
                // Minutes the current trip must wait before the pump is free
                $waitingTime = (int) ceil($currentPouringStart->diffInMinutes($prevEnd));

                if ($waitingTime > 0) {
                    // waiting sits right where pouring would have started
                    $waitingStart = $currentPouringStart->copy();       // insp_end + 1 min
                    $waitingEnd   = $prevEnd->copy()->subMinute();       // one minute before pump free

                    // New pouring aligns exactly to previous pouring_end (no gap/overlap)
                    $newPouringStart = $prevEnd->copy();
                    $pouringDuration = (int) ($trip['pouring_time'] ?? 0);
                    $newPouringEnd   = $newPouringStart->copy()->addMinutes($pouringDuration);

                    // Cascade: cleaning and return shift by the same waiting amount
                    $cleaningTime = (int) ($trip['cleaning_time'] ?? $scheduleData->cleaning_time ?? 0);
                    $returnTime   = (int) ($trip['return_time_mins']
                        ?? ($trip['return_start'] instanceof Carbon
                            ? $trip['return_start']->diffInMinutes($trip['return_end'])
                            : Carbon::parse($trip['return_start'])->diffInMinutes(Carbon::parse($trip['return_end']))));

                    $newCleaningStart = $newPouringEnd->copy()->addMinute();
                    $newCleaningEnd   = $newCleaningStart->copy()->addMinutes($cleaningTime);
                    $newReturnStart   = $newCleaningEnd->copy()->addMinute();
                    $newReturnEnd     = $newReturnStart->copy()->addMinutes($returnTime);

                    // Commit adjusted timings to scheduleData
                    $scheduleData->waiting_start  = $waitingStart;
                    $scheduleData->waiting_end    = $waitingEnd;
                    $scheduleData->waiting_time   = $waitingTime;
                    $scheduleData->pouring_start  = $newPouringStart;
                    $scheduleData->pouring_end    = $newPouringEnd;
                    $scheduleData->cleaning_start = $newCleaningStart;
                    $scheduleData->cleaning_end   = $newCleaningEnd;
                    $scheduleData->return_start   = $newReturnStart;
                    $scheduleData->return_end     = $newReturnEnd;

                    Log::info(sprintf(
                        "[WAITING] order=%s trip=%d pump_overlap detected — "
                            . "prev_pouring_end=%s original_pouring_start=%s "
                            . "waiting=%dmin waiting_start=%s waiting_end=%s "
                            . "new_pouring_start=%s new_pouring_end=%s",
                        $trip['order_no'] ?? '?',
                        $trip['trip'] ?? 0,
                        $prevEnd->format('H:i'),
                        $currentPouringStart->format('H:i'),
                        $waitingTime,
                        $waitingStart->format('H:i'),
                        $waitingEnd->format('H:i'),
                        $newPouringStart->format('H:i'),
                        $newPouringEnd->format('H:i')
                    ));
                }
            }
        }
    ...
    private function simulatePlantAssignment(ScheduleData $scheduleData, $feasibleOrders, int $shiftMinutes): array
    {
        ...
        foreach ($sorted as $order) {
            $loadingTime      = (int) ($order->loading_time ?? ConstantHelper::LOADING_TIME);
            $maxInterval      = $order->max_interval;
            $truckCapacity    = self::DEFAULT_TRUCK_CAPACITY;
            $totalTrips       = (int) ceil($order->quantity / max(1, $truckCapacity));
