...
class ScheduleService
{
    ...
    private function generateAllOrderTrips(ScheduleData $scheduleData, $allOrders): array
    {
        ...
        foreach ($allOrders as $order) {
            ...
            for ($trip = 1; $trip <= $totalTrips; $trip++) {
                ...
                if ($trip === 1) {
                    ...
                    $loadingStart = $previousLoadingEnd->copy()->addMinute();
                }
        ...
        usort($allTrips, function ($a, $b) {
            ...
            //     return $a['order_lpi_score'] <=> $b['order_lpi_score'];
            // }
        ...
        $trip1 = collect($allTrips)
            ->where('order_no', 111891)
            ->map(fn($t, $i) => sprintf(
                "#%d | End: %s ",
                $i + 1,
                \Carbon\Carbon::parse($t['return_end'])->format('H:i'),
            ))
            ->values();
...
//        dd($trip1->toArray());
    ...
    private function scheduleTripsChronologically(ScheduleData &$scheduleData, array $sortedTrips, $allOrders): void
    {
        ...
        $maxRetryMinutes = $scheduleData->max_delay_minutes ?? 900;
        ...
        foreach ($sortedTrips as $tripData) {
            ...
            while ($retryOffset <= $maxRetryMinutes) {
                ...
                // if ($order->order_no === "111891")
                //     dd($lastTripReturnEnd);
                if ($scheduleData->trip === 1) {
                    ...
                    if ($order->pump) {
                        ...
                        // if ($order->order_no === "114321")
                        //     dd($delay, $scheduleData->pump_busy_slots_unset);
                        $scheduleData->pump_busy_slots_unset[] = [
                            ...
                            'end'      =>  $lastTripReturnEnd
                                ...
                                : $delay['pouring_start'], // fallback
                            'truck_id' => null,
                            'cap'      => null,
                            'order_no' => $orderNo,
                            'pump_id' => $delay['pump_id']
                        ];
                        Log::info("[PUMP] Assigned pump_id={$delay['pump_id']} for order={$orderNo} "
                            . "busy from " . $delay['pump_qc_start']->format('H:i') . " to " . Carbon::parse($lastTripReturnEnd)->format('H:i'));
                    }
                }
                if ($scheduleData->trip === $totalTrips) {
                    if ($order->pump) {
                        // Find and update the busy slot end for this order's pump
                        foreach ($scheduleData->pump_busy_slots_unset as &$busySlot) {
                            if ($busySlot['order_no'] === $orderNo) {
                                $busySlot['end'] = $scheduleData->return_end->copy(); // final trip's return_end
                                Log::info("[PUMP] Updated busy slot end for order={$orderNo} "
                                    . "end=" . $busySlot['end']->format('H:i'));
                                break;
                            }
                        }
                        unset($busySlot); // break reference
                    }
                }
    ...
    /**
     * Reschedule an order if pump is not available.
     * Finds next free pump slot and re-runs full trip generation + scheduling.
     */
    private function rescheduleOrderForPump(
        ScheduleData &$scheduleData,
        $order,
        ...
        Log::info("[PUMP_RESCHEDULE] Starting pump reschedule for order={$orderNo}");

        // ── Step 1: Find next free pump slot ─────────────────────────────────────
        $nextFreePumpTime = $this->getNextFreePumpSlot($scheduleData, $order, Carbon::parse($order->delivery_date));

        if (!$nextFreePumpTime) {
            Log::error("[PUMP_RESCHEDULE] No free pump slot found for order={$orderNo}");
            $orderFailed[$orderNo] = "No available pump slot found for order={$orderNo}";
            return;
        }

        Log::info("[PUMP_RESCHEDULE] order={$orderNo} next free pump slot="
            . $nextFreePumpTime['pouring_start']->format('H:i'));

        // ── Step 2: Calculate how much to shift the order ────────────────────────
        // Original pouring start (trip 1)
        $originalPouringStart = Carbon::parse($order->delivery_date);

        // New delivery time = next free pump slot
        $shiftMinutes = $originalPouringStart->diffInMinutes($nextFreePumpTime['pouring_start'], false);

        if ($shiftMinutes <= 0) {
            Log::info("[PUMP_RESCHEDULE] order={$orderNo} pump slot is in the past, "
                . "no reschedule needed");
            return;
        }

        Log::info("[PUMP_RESCHEDULE] order={$orderNo} shifting delivery by +{$shiftMinutes} mins "
            . "from {$originalPouringStart->format('H:i')} "
            . "to {$nextFreePumpTime['pouring_start']->format('H:i')}");

        // ── Step 3: Update order delivery date to new pump-free time ─────────────
        $order->delivery_date = $nextFreePumpTime['pouring_start']->format('Y-m-d H:i:s');

        // ── Step 4: Clear any previously scheduled trips for this order ──────────
        unset($orderSchedules[$orderNo]);
        unset($orderDelivered[$orderNo]);
        unset($orderFailed[$orderNo]);
        unset($orderCumulativeDelay[$orderNo]);
        unset($bestPlantPerOrderTrip[$orderNo]);

        // ── Step 5: Re-generate all trips for this order with new delivery time ───
        $newTrips = $this->generateTripsForSingleOrder($scheduleData, $order);

        if (empty($newTrips)) {
            Log::error("[PUMP_RESCHEDULE] order={$orderNo} failed to generate new trips");
            $orderFailed[$orderNo] = "Trip generation failed after pump reschedule for order={$orderNo}";
            return;
        }

        Log::info("[PUMP_RESCHEDULE] order={$orderNo} generated " . count($newTrips)
            . " new trips starting at "
            . Carbon::parse($newTrips[0]['loading_start'])->format('H:i'));

        // ── Step 6: Schedule the new trips one by one ─────────────────────────────
        foreach ($newTrips as $tripData) {
            $priorDelay   = $orderCumulativeDelay[$orderNo] ?? 0;
            $retryOffset  = 0;
            $tripScheduled = false;
            ...
            $maxRetryMinutes = $scheduleData->max_delay_minutes ?? 600;
            ...
            while ($retryOffset <= $maxRetryMinutes) {
                ...
                if (Carbon::parse($currentTrip['loading_start'])
                    ->gt(Carbon::parse($scheduleData->shift_end))
                ) {
                    $orderFailed[$orderNo] = "order={$orderNo} trip={$tripData['trip']} "
                        . "exceeded shift_end after pump reschedule.";
                    Log::warning("[PUMP_RESCHEDULE] " . $orderFailed[$orderNo]);
                    break;
                }

                // Set scheduleData from current trip
                $scheduleData->order_no       = $orderNo;
                $scheduleData->location       = $tripData['location'];
                $scheduleData->loading_start  = $currentTrip['loading_start'];
                $scheduleData->loading_end    = $currentTrip['loading_end'];
                $scheduleData->return_end     = $currentTrip['return_end'];
                $scheduleData->loading_time   = $currentTrip['loading_time'];
                ...
                if ($scheduleData->trip === 1) {
                    $bestPlantPerOrderTrip[$orderNo] =
                        $scheduleData->batching_plant['data']['plant_name'];
                }
                ...
                $orderSchedules[$orderNo][]  = $entry;
                $orderDelivered[$orderNo]    = ($orderDelivered[$orderNo] ?? 0)
                    + $currentTrip['batching_qty'];

                $this->updateResourcePoolsOnly($scheduleData, $order, $tripData['location']);

                if ($retryOffset > 0) {
                    $orderCumulativeDelay[$orderNo] = $priorDelay + $retryOffset;
                }

                Log::info("[PUMP_RESCHEDULE] ✓ order={$orderNo} trip={$currentTrip['trip']}"
                    . " loading_start=" . $currentTrip['loading_start']->format('H:i')
                    . " pouring_start=" . $currentTrip['pouring_start']->format('H:i'));
            ...
            if (!$tripScheduled) {
                $orderFailed[$orderNo] = "order={$orderNo} trip={$tripData['trip']} "
                    . "could not schedule after pump reschedule + {$maxRetryMinutes}min retry.";
                Log::error("[PUMP_RESCHEDULE] ✗ " . $orderFailed[$orderNo]);
                break; // stop further trips for this order
            }
        }
    }
    ...
    /**
     * Generate all trips for a single order (extracted from generateAllOrderTrips).
     * Used when rescheduling a single order after pump unavailability.
     */
    private function generateTripsForSingleOrder(ScheduleData $scheduleData, $order): array
    {
        $trips         = [];
        $truckCapacity = self::DEFAULT_TRUCK_CAPACITY;
        $loadingTime   = $order->loading_time  ?? ConstantHelper::LOADING_TIME;
        ...
        for ($trip = 1; $trip <= $totalTrips; $trip++) {
            ...
            Log::info("[GEN_SINGLE_TRIP] order={$order->order_no} trip={$trip}/{$totalTrips} "
                . "loading_start=" . $loadingStart->format('H:i')
                . " pouring_start=" . $pouringStart->format('H:i'));
        }
    ...
    private function getNextFreeTruckSlot(ScheduleData $scheduleData, $order): ?Carbon
    {
        // Find all busy slots for trucks matching this order's requirements
        $relevantSlots = collect($scheduleData->truck_busy_slots)->sortBy(fn($s) => Carbon::parse($s['end'])->timestamp)
            ->values();

        if ($relevantSlots->isEmpty()) {
            // No busy slots = truck is free right now = no change needed
            return null;
        }

        // Next free = latest return_end among matching truck slots
        $latestEnd = $relevantSlots
            ->max(fn($s) => Carbon::parse($s['end'])->timestamp);

        $nextFree = Carbon::createFromTimestamp($latestEnd)->addMinute();

        // Safety: must be within shift
        if ($nextFree->gt(Carbon::parse($scheduleData->shift_end))) {
            Log::warning("[PUMP_RESCHEDULE] Next free truck slot {$nextFree->format('H:i')} "
                . "is beyond shift end");
            return null;
        }
